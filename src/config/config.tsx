...
const Config: FC = () => {
  ...
  if (!_.isEmpty(settingInfo)) {
    const headerInfo = kintone.plugin.app.getProxyConfig(settingInfo!.url, 'POST')
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <p className="kintoneplugin-row">
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <TextReact defaultValue={defaultToken} label="token信息:" {...register('token')} {...field}></TextReact>
              )}
            />
          </p>
          <p className="kintoneplugin-row">
            <Controller
              name="url"
              control={control}
              render={({ field }) => <TextReact label="接口的地址:" {...register('url')} {...field}></TextReact>}
            />
          </p>
          <p className="kintoneplugin-row">
            <Controller
              name="subId"
              control={control}
              render={({ field }) => <TextReact label="订阅应用id:" {...register('subId')} {...field}></TextReact>}
            />
          </p>
          <p className="kintoneplugin-row">
            <Controller
              name="userId"
              control={control}
              render={({ field }) => <TextReact label="用户应用id:" {...register('userId')} {...field}></TextReact>}
            />
          </p>
          <p className="kintoneplugin-row">
            <ButtonReact className="my-button mr-4" onClick={handleCancel} text="取消" />
            <ButtonReact className="my-button" onClick={handleSubmit(onSubmit)} text="保存" type="submit" />
          </p>
        </form>
      </div>
    </>
